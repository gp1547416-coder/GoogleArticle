...
export default function TechWikiArticle() {
  return (
    <div className="bg-[#f2f2f2] min-h-screen font-sans text-[#333] w-full">
      {/* Top Navbar */}
      <header className="bg-[#3e8e41] text-white p-3 shadow-md w-full">
        <div className="max-w-5xl mx-auto flex justify-between items-center px-4">
          <div className="flex items-center gap-4">
            <Menu className="w-6 h-6" />
            <h1 className="text-3xl font-bold italic tracking-tighter">TechWiki</h1>
          </div>
          <div className="flex-1 max-w-md mx-8 hidden sm:block">
            <div className="relative">
              <input 
                ...
                className="w-full py-1 px-4 rounded-full text-black outline-none border-none"
              />
      ...
      {/* Breadcrumbs */}
      <nav className="max-w-5xl mx-auto py-4 px-6 text-xs text-gray-500 flex items-center gap-1">
        <span>Home</span> <ChevronRight className="w-3 h-3" />
        <span>Computers and Electronics</span> <ChevronRight className="w-3 h-3" />
        <span className="text-[#3e8e41] font-semibold">Internet</span>
      </nav>

      {/* Main Content Card */}
      <main className="max-w-5xl mx-auto bg-white rounded-t-xl shadow-lg overflow-hidden border border-gray-200 mb-10">
        <div className="p-6 md:p-12">
          ...
          <div className="flex items-center gap-2 mb-8 border-b pb-6">
            <span className="bg-[#3e8e41] text-white px-2 py-1 rounded text-[10px] font-bold uppercase">Co-authored by</span>
            <span className="text-[#3e8e41] font-bold hover:underline cursor-pointer">TechWiki Staff</span>
          </div>

          {/* Article Body */}
          <section className="mb-10">
            <div className="bg-[#f1f8f1] border-l-8 border-[#3e8e41] p-6 md:p-8 rounded-r-lg">
              <h2 className="text-2xl font-bold mb-4 text-[#3e8e41]">Method 1: Mastering the Search Ecosystem</h2>
          ...
          {/* Feedback Section */}
          <div className="border-t pt-8 mt-12 flex flex-col items-center gap-4">
            <h3 className="text-xl font-bold">Did this article help you?</h3>
            <div className="flex gap-4">
              <button className="flex items-center gap-2 px-8 py-2 border border-gray-300 rounded-full hover:bg-gray-50 transition font-medium">
                <ThumbsUp className="w-5 h-5 text-[#3e8e41]" /> Yes
              </button>
              <button className="flex items-center gap-2 px-8 py-2 border border-gray-300 rounded-full hover:bg-gray-50 transition font-medium">
      ...
      <footer className="max-w-5xl mx-auto py-12 text-center text-gray-400 text-sm">
        <p>© 2026 TechWiki. All rights reserved.</p>
      </footer>
    </div>
  );
}
